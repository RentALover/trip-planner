package com.tripplanner.module.export.service.impl;

import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;
import com.tripplanner.common.BusinessException;
import com.tripplanner.module.day.entity.TripDay;
import com.tripplanner.module.day.mapper.DayMapper;
import com.tripplanner.module.export.service.ExportService;
import com.tripplanner.module.item.entity.TripItem;
import com.tripplanner.module.item.mapper.ItemMapper;
import com.tripplanner.module.transport.entity.ItemTransport;
import com.tripplanner.module.transport.mapper.TransportMapper;
import com.tripplanner.module.trip.entity.Trip;
import com.tripplanner.module.trip.mapper.TripMapper;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Service;

import java.math.BigDecimal;
import java.time.format.DateTimeFormatter;
import java.util.List;

@Service
@RequiredArgsConstructor
public class ExportServiceImpl implements ExportService {

    private final TripMapper tripMapper;
    private final DayMapper dayMapper;
    private final ItemMapper itemMapper;
    private final TransportMapper transportMapper;

    private static final DateTimeFormatter DATE_FMT = DateTimeFormatter.ofPattern("MM/dd");
    private static final DateTimeFormatter TIME_FMT = DateTimeFormatter.ofPattern("HH:mm");

    @Override
    public String exportText(Long userId, Long tripId) {
        Trip trip = validateOwnership(userId, tripId);

        StringBuilder sb = new StringBuilder();
        sb.append("═══════════════════════════════════\n");
        sb.append("  ").append(trip.getTripName()).append("\n");
        sb.append("═══════════════════════════════════\n");
        sb.append("目的地: ").append(trip.getDestination()).append("\n");
        sb.append("日期: ").append(trip.getStartDate()).append(" ~ ").append(trip.getEndDate()).append("\n");
        sb.append("人数: ").append(trip.getNumPeople()).append("人\n");
        if (trip.getNotes() != null && !trip.getNotes().isEmpty()) {
            sb.append("备注: ").append(trip.getNotes()).append("\n");
        }
        sb.append("预算: ").append(trip.getTotalBudget() != null ? "¥" + trip.getTotalBudget() : "未设定").append("\n\n");

        List<TripDay> days = dayMapper.selectList(
                new LambdaQueryWrapper<TripDay>()
                        .eq(TripDay::getTripId, tripId)
                        .orderByAsc(TripDay::getDayNumber));

        for (TripDay day : days) {
            sb.append("──────────────────────────────────\n");
            sb.append("Day ").append(day.getDayNumber())
                    .append(" - ").append(day.getDate().format(DATE_FMT)).append("\n");
            if (day.getNotes() != null && !day.getNotes().isEmpty()) {
                sb.append("  备注: ").append(day.getNotes()).append("\n");
            }
            sb.append("\n");

            List<TripItem> items = itemMapper.selectList(
                    new LambdaQueryWrapper<TripItem>()
                            .eq(TripItem::getDayId, day.getId())
                            .orderByAsc(TripItem::getSortOrder));

            for (int i = 0; i < items.size(); i++) {
                TripItem item = items.get(i);
                String typeLabel = getTypeLabel(item.getItemType());
                sb.append("  [").append(typeLabel).append("] ").append(item.getTitle()).append("\n");
                if (item.getStartTime() != null) {
                    sb.append("    时间: ").append(item.getStartTime().format(TIME_FMT));
                    if (item.getEndTime() != null) {
                        sb.append(" ~ ").append(item.getEndTime().format(TIME_FMT));
                    }
                    sb.append("\n");
                }
                if (item.getLocation() != null) {
                    sb.append("    地点: ").append(item.getLocation()).append("\n");
                }
                if (item.getCost() != null && item.getCost().compareTo(BigDecimal.ZERO) > 0) {
                    sb.append("    费用: ¥").append(item.getCost()).append("\n");
                }
                if (item.getDescription() != null && !item.getDescription().isEmpty()) {
                    sb.append("    备注: ").append(item.getDescription()).append("\n");
                }

                // Look for transport to next item
                if (i < items.size() - 1) {
                    ItemTransport transport = transportMapper.selectOne(
                            new LambdaQueryWrapper<ItemTransport>()
                                    .eq(ItemTransport::getDayId, day.getId())
                                    .eq(ItemTransport::getFromItemId, item.getId())
                                    .eq(ItemTransport::getToItemId, items.get(i + 1).getId()));
                    if (transport != null) {
                        sb.append("    ↓ [").append(getTransportLabel(transport.getTransportType())).append("]");
                        if (transport.getEstimatedDuration() != null) {
                            sb.append(" 约").append(transport.getEstimatedDuration()).append("分钟");
                        }
                        if (transport.getCost() != null && transport.getCost().compareTo(BigDecimal.ZERO) > 0) {
                            sb.append(" ¥").append(transport.getCost());
                        }
                        if (transport.getRouteInfo() != null && !transport.getRouteInfo().isEmpty()) {
                            sb.append(" (").append(transport.getRouteInfo()).append(")");
                        }
                        sb.append("\n");
                    }
                }
                sb.append("\n");
            }
        }

        sb.append("═══════════════════════════════════\n");
        return sb.toString();
    }

    @Override
    public String exportJson(Long userId, Long tripId) {
        Trip trip = validateOwnership(userId, tripId);
        // For JSON export, we rely on the existing APIs to serialize
        // This is a placeholder for future implementation
        return "{\"tripId\":" + trip.getId() + "}";
    }

    private Trip validateOwnership(Long userId, Long tripId) {
        Trip trip = tripMapper.selectById(tripId);
        if (trip == null) {
            throw BusinessException.notFound("行程不存在");
        }
        if (!trip.getUserId().equals(userId)) {
            throw BusinessException.forbidden("无权操作此行程");
        }
        return trip;
    }

    private String getTypeLabel(String type) {
        return switch (type) {
            case "TRANSPORT" -> "交通";
            case "ACCOMMODATION" -> "住宿";
            case "DINING" -> "餐饮";
            case "ATTRACTION" -> "景点";
            case "SHOPPING" -> "购物";
            default -> "其他";
        };
    }

    private String getTransportLabel(String type) {
        return switch (type) {
            case "WALK" -> "步行";
            case "BUS" -> "公交";
            case "SUBWAY" -> "地铁";
            case "TAXI" -> "出租车";
            case "RIDE_HAIL" -> "网约车";
            case "SELF_DRIVE" -> "自驾";
            case "BIKE" -> "单车";
            default -> "交通";
        };
    }
}
